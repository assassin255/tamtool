...
build_qemu=$(ask "👉 Có build QEMU TCG không? (y/n): " "y")

if [[ "$build_qemu" == "y" ]]; then

sudo apt update -y
sudo apt install -y wget git ninja-build build-essential pkg-config meson python3 python3-venv python3-pip libglib2.0-dev libpixman-1-dev zlib1g-dev libslirp-dev clang-15 lld-15 llvm-15 llvm-15-dev libepoxy-dev libdrm-dev libegl1 libgl1 libgbm1 mesa-utils aria2 tmux libvirglrenderer-dev
...
pip install --upgrade pip packaging tomli
...
EXTRA_CFLAGS="-Ofast -march=native -mtune=native -pipe -flto=full -fuse-ld=lld -fno-plt -fomit-frame-pointer -fno-unwind-tables -fno-asynchronous-unwind-tables -fno-stack-protector -funroll-loops -finline-functions -DNDEBUG"
LDFLAGS="-flto=full -Wl,--lto-O3 -Wl,--gc-sections -Wl,--icf=all"

../qemu-src/configure --prefix=/opt/qemu-optimized --target-list=x86_64-softmmu --enable-tcg --enable-slirp --enable-lto --enable-opengl --enable-virglrenderer --disable-kvm --disable-gtk --disable-sdl --disable-vnc --disable-spice --disable-debug-info --disable-docs CC="$CC" CXX="$CXX" LD="$LD" CFLAGS="$EXTRA_CFLAGS" CXXFLAGS="$EXTRA_CFLAGS" LDFLAGS="$LDFLAGS"
...
read -rp "👉 Nhập số: " win_choice

case "$win_choice" in
1) WIN_URL="https://drive.muavps.net/file/Windows2012.img" ;;
2) WIN_URL="https://drive.muavps.net/file/Windows2016.img" ;;
...
read -rp "📦 Tăng dung lượng đĩa (GB, default 20): " extra_gb
