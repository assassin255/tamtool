...
echo "🚀 Build QEMU 10.2.0 STABLE + LLVM15 ULTRA-Optimized (HEADLESS)"
sudo apt update -y
sudo apt install -y wget gnupg lsb-release software-properties-common build-essential ninja-build git python3 python3-venv python3-pip libglib2.0-dev libpixman-1-dev zlib1g-dev libfdt-dev libslirp-dev pkg-config meson aria2 clang-15 lld-15 llvm-15 llvm-15-dev llvm-15-tools
...
EXTRA_CFLAGS="-O3 -Ofast -march=native -mtune=native -pipe -flto=full -fuse-ld=lld -fno-semantic-interposition -fno-plt -fno-unwind-tables -fno-stack-protector -funroll-loops -finline-functions -ffast-math -fstrict-aliasing -DNDEBUG -DDEFAULT_TCG_TB_SIZE=2097152 -DTCG_ACCEL_FAST=1 -DTCG_OVERSIZED_OP=1 -DTCG_ENABLE_FAST_PREFETCH=1 -DQEMU_STRICT_ALIGN=0"

LDFLAGS="-flto=full -Wl,--lto-O3 -Wl,--icf=all -Wl,--gc-sections"
...
-machine q35 \
-cpu "$cpu_model" \
-smp "$cpu_core",threads=1,cores="$cpu_core" \
-m "${ram_size}G" \
...
for i in {1..10}; do
PUBLIC=$(tmux capture-pane -pt kami -p | sed 's/\x1b[[0-9;]*m//g' | grep -oE '([a-zA-Z0-9.-]+):(3389)' | head -n1)
[ -n "$PUBLIC" ] && break
sleep 2
done

if [ -z "$PUBLIC" ]; then
echo "❌ Tunnel không cấp public IP"
tmux capture-pane -pt kami -p | tail -n 20
else
